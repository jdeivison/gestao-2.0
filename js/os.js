const bancoDadosFake = {
    marcas: ["Samsung", "Apple", "Dell", "HP", "Lenovo", "Asus", "Acer", "Positivo"],
    modelos: ["Galaxy S23 Ultra", "iPhone 15 Pro", "Inspiron 15", "ThinkPad T14", "Macbook Air M2"],
    historicoServicos: []
};
...
function toggleTheme() {
    ...
    document.getElementById('theme-toggle').innerText = isDark ? '☀️ Claro' : '🌙 Escuro';
}
...
function buscarHistorico(tipo) {
    ...
    document.getElementById('modal-titulo').innerText = `Buscar ${tipo.toUpperCase()}`;
    lista.innerHTML = "";
    modal.style.display = "block";

    let dados = (tipo === 'marca') ? bancoDadosFake.marcas : 
               (tipo === 'modelo') ? bancoDadosFake.modelos : bancoDadosFake.historicoServicos;
    
    dados.forEach(item => {
        const li = document.createElement('li');
        if (typeof item === 'string') {
            li.innerText = item;
            li.onclick = () => { document.getElementById(tipoBuscaAtivo).value = item; fecharModal(); };
        } else {
...
function filtrarBusca() {
    ...
    itens.forEach(li => li.style.display = li.innerText.toLowerCase().includes(termo) ? "block" : "none");
}
...
function atualizarDashboard() {
    ...
    let saldo = financeiro.reduce((acc, current) => current.tipo === 'Entrada' ? acc + current.valor : acc - current.valor, 0);
    document.getElementById('dash-saldo').innerText = `R$ ${saldo.toLocaleString('pt-BR', {minimumFractionDigits: 2})}`;

    document.getElementById('bar-os').style.width = Math.min(historico.length * 10, 100) + "%";
    document.getElementById('bar-saldo').style.width = Math.min((saldo / 5000) * 100, 100) + "%";
    document.getElementById('bar-pendencia').style.width = Math.min(remessas.length * 20, 100) + "%";
}
...
function renderizarEstoque() {
    ...
    document.getElementById('corpo-estoque').innerHTML = lista.map(i => `<tr><td>${i.nome}</td><td>${i.qtd} un</td><td>${i.data}</td><td><button class="btn-search" onclick="gerarRemessaManual('${i.nome}')">🚛</button></td></tr>`).join('');
}
...
function salvarNoFinanceiro(desc, valor, tipo) {
    ...
    renderizarFinanceiro(); atualizarDashboard();
}

function renderizarFinanceiro() {
    const lista = JSON.parse(localStorage.getItem('financeiro_dados')) || [];
    document.getElementById('corpo-financeiro').innerHTML = lista.map(f => `<tr><td>${f.desc}</td><td style="color:${f.tipo === 'Entrada' ? '#2ecc71' : '#e74c3c'}">R$ ${f.valor.toFixed(2)}</td><td>${f.tipo}</td><td>${f.data}</td></tr>`).join('');
}

function gerarRemessaManual(item) {
    const motivo = prompt("Motivo (Garantia, Conserto ou Devolução):", "Garantia");
    if(!motivo) return;
    let remessas = JSON.parse(localStorage.getItem('remessas')) || [];
    remessas.push({ item, motivo, status: 'Em Trânsito', data: new Date().toLocaleDateString() });
    localStorage.setItem('remessas', JSON.stringify(remessas));
    renderizarRemessas(); atualizarDashboard();
}

function renderizarRemessas() {
    const lista = JSON.parse(localStorage.getItem('remessas')) || [];
    document.getElementById('corpo-remessas').innerHTML = lista.map(r => `<tr><td>${r.item}</td><td><span class="badge">${r.motivo}</span></td><td>${r.status}</td><td>${r.data}</td></tr>`).join('');
}

function converterEmVenda() {
    const dados = {
        marca: document.getElementById('marca').value, modelo: document.getElementById('modelo').value,
        serie: document.getElementById('serie').value, lacre: document.getElementById('lacre').value,
        etiqueta: document.getElementById('etiqueta').value, inventario: document.getElementById('inventario').value,
    ...
    win.document.write(`<html><body style="font-family:sans-serif; padding:40px;"><h1>NOTA DE VENDA</h1><p>Cliente: ${dados.cliente}</p><p>Valor: R$ ${valor.toFixed(2)}</p><hr><h2>VERSO (EQUIPAMENTO)</h2><p>Marca: ${dados.marca} | Série: ${dados.serie}</p><p>Lacre: ${dados.lacre} | Etiqueta: ${dados.etiqueta}</p><script>window.print();</script></body></html>`);
    win.document.close();
}
